...
class GLMModel:
   ...
   Extensions:
     ...
     - evaluate_mlm_quality_on_val (loss + perplexity on VAL)
   ...
   def _cleanup_all(self):
       """Delete model_path better using only os.*"""
   ...
   def _ensure_special_tokens(self):
       """Fix cases where HF loads tokenizer but token attributes are None."""
   ...
   def get_split_indices(self) -> Dict[str, np.ndarray]:
       ...
       if not os.path.exists(split_path):
           raise RuntimeError(
               ...
               f"Train once with save_split=True to create them."
           )
       data = np.load(split_path)
       return {"train_idx": data["train_idx"], "val_idx": data["val_idx"]}
   ...
   def train(
       ...
       args = self._make_training_args(
           ...
           eval_steps=500,
           save_strategy="steps",
           load_best_model_at_end=False,  # <-- key change
       ...
       # Manually reload best checkpoint (clean) and "tie" weights to avoid warnings
       best_path = getattr(trainer.state, "best_model_checkpoint", None)
       if best_path is not None and os.path.isdir(best_path):
           best_model = BertForMaskedLM.from_pretrained(best_path)
           try:
               best_model.tie_weights()
           except Exception:
               pass
   ...
   def delta_likelihood_fast(
       ...
       tok_start = start + 1  # +1 for [CLS]
       tok_end = end + 1

       max_token_index = ref_inputs.input_ids.shape[1] - 2  # last before [SEP]
